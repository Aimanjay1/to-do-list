...
export default function App() {
    ...
    const check = () => {
      ...
      tasks
        ...
        .forEach((task) => {
          ...
          if (
            diff > 0 &&
            diff <= task.reminderBefore &&
            !notifiedRef.current.has(task.id)
          ) {
            notifiedRef.current.add(task.id);
            addToast(
              `⏰ "${task.title}" due in ${Math.round(diff)} min!`,
              "warning",
            );
          }
        });
  ...
  const saveTask = (form) => {
    if (editingTask) {
      setTasks((p) =>
        p.map((t) => (t.id === editingTask.id ? { ...t, ...form } : t)),
      );
  ...
  const cancelTask = (id) => {
    setTasks((p) =>
      p.map((t) => (t.id === id ? { ...t, cancelled: true } : t)),
    );
  ...
  const changeStatus = (taskId, ns) => {
    setTasks((p) =>
      p.map((t) => {
        if (t.id !== taskId || t.kanbanStatus === ns) return t;
        const col = KANBAN_COLS.find((c) => c.key === ns);
        addToast(`Moved to "${col?.label}"`, "success");
        if (ns === "done")
          return {
            ...t,
            kanbanStatus: ns,
        ...
        return { ...t, kanbanStatus: ns };
      }),
    );
  };
  const changePriority = (taskId, np) => {
    setTasks((p) =>
      p.map((t) => {
        if (t.id !== taskId || t.priority === np) return t;
        addToast(`Moved to "${PRIORITY_CONFIG[np].desc}"`, "success");
        return { ...t, priority: np };
      }),
    );
  };
  ...
  const saveFolder = (data) => {
    setFolders((p) => [...p, { id: `f_${Date.now()}`, ...data }]);
    addToast(`"${data.name}" created!`, "success");
    setShowFolderModal(false);
  };
  const pendingCount = (fid) =>
    tasks.filter(
      (t) =>
        !t.completed && !t.cancelled && (fid === "all" || t.folderId === fid),
    ).length;
    ...
    if (curFolder.boardType === "kanban")
      return (
        <KanbanBoard
          ...
          onNewTask={(ck) =>
            openNewTask({ folderId: selectedFolder, kanbanStatus: ck })
          }
        />
      );
    if (curFolder.boardType === "matrix")
      return (
        <MatrixBoard
          tasks={boardTasks}
          folders={folders}
          ...
          onNewTask={(qk) =>
            openNewTask({ folderId: selectedFolder, priority: qk })
          }
        />
      );
    return (
  ...
  const boardLabel =
    curFolder?.boardType === "kanban"
      ? "⚙️ Kanban"
      : curFolder?.boardType === "matrix"
        ? "⊞ Matrix"
        : null;
    ...
    <>
          ...
          <div style={{ padding: "14px 12px", flex: 1, overflowY: "auto" }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 10,
                border: "none",
                background: isHome ? "rgba(245,158,11,0.12)" : "transparent",
                color: isHome ? "#f59e0b" : "#94a3b8",
                cursor: "pointer",
                textAlign: "left",
                fontSize: 14,
            ...
            {folders.map((folder) => (
                ...
                style={{
                  ...
                  background:
                    selectedFolder === folder.id
                      ...
                      : "transparent",
                  color:
                    selectedFolder === folder.id ? folder.color : "#94a3b8",
                  cursor: "pointer",
                  textAlign: "left",
                  fontSize: 13,
                  transition: "all 0.15s",
              ...
              >
                ...
                >
                  {folder.boardType === "kanban" ? "KB" : "MX"}
                </span>
                {pendingCount(folder.id) > 0 && (
                  <span
                    style={{
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                {[
                  {
                    label: "Active",
                    count: tasks.filter((t) => !t.completed && !t.cancelled)
                      .length,
                    color: "#3b82f6",
                  },
                  {
                  ...
                  {
                    label: "Cancel",
                    count: tasks.filter((t) => t.cancelled).length,
                    color: "#6b7280",
                  },
                ].map((s) => (
        ...
        >
            ...
            <>
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  >
                    {curFolder && (
                      <span style={{ fontSize: 22 }}>{curFolder.icon}</span>
                    )}
                    <h1
                      style={{
                        fontFamily: "'Playfair Display',serif",
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  <div style={{ position: "relative" }}>
                    ...
                    <input
                      ...
                      placeholder="Search..."
                      style={{ ...inputStyle, paddingLeft: 34, width: 170 }}
                    />
                  </div>
                  <button
                    onClick={() =>
                      openNewTask(curFolder ? { folderId: selectedFolder } : {})
                    }
                    style={{
                      padding: "9px 18px",
                      background: "linear-gradient(135deg,#f59e0b,#ef4444)",
              ...
              >
                {[
                  [
                    "board",
                    curFolder?.boardType === "kanban" ? "⚙️ Board" : "⊞ Board",
                  ],
                  ["history", "📋 History"],
                ].map(([k, lbl]) => (
                  <button
                    ...
                    style={{
                      ...
                      border: "none",
                      background: "none",
                      color: activeTab === k ? "#f59e0b" : "#64748b",
                      cursor: "pointer",
                      fontSize: 13,
                      fontWeight: 500,
                      borderBottom: `2px solid ${activeTab === k ? "#f59e0b" : "transparent"}`,
                      fontFamily: "'DM Sans',sans-serif",
                      transition: "all 0.2s",
                    }}
                  >
                    {lbl}
                    {k === "history" &&
                      tasks.filter((t) => t.completed || t.cancelled).length >
                        0 && (
                        <span
                          style={{
                            marginLeft: 7,
                        ...
                        >
                          {
                            tasks.filter((t) => t.completed || t.cancelled)
                              .length
                          }
                        </span>
                      )}
          ...
          <div
            style={{
              flex: 1,
              overflowY:
                curFolder?.boardType === "kanban" && activeTab === "board"
                  ? "hidden"
                  : "auto",
              overflowX: "hidden",
              padding: isHome ? "0" : "24px 28px",
      ...
      {showTaskModal && (
        <TaskModal
          task={editingTask}
          folders={folders}
          defaultFolderId={newTaskDefaults.folderId || selectedFolder}
          onSave={(form) =>
            saveTask({ ...form, ...(!editingTask ? newTaskDefaults : {}) })
          }
          onClose={() => {
            setShowTaskModal(false);
            setEditingTask(null);
